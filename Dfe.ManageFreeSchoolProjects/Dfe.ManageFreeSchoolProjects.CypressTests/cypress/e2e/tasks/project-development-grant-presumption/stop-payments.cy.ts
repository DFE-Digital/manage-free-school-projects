...
describe('Stop Payments Task', () => {
    ...
    it('Should successfully set Stop Payments', () => {
        ...
        stopPayments.titleIs('Edit Stop payments').schoolNameIs(project.schoolName).clickContinue();
        ...
        stopPayments.withPaymentStopped('Yes').withPaymentStoppedDate('5', '12', '2025').clickContinue();
        ...
        stopPayments.withPaymentStopped('No').clickContinue();

        summaryPage.inOrder().skip(8).summaryShows('Are you sure you want to stop payments?').HasValue('No');

        pdgDashboard.selectChangeStopPayments();

        stopPayments.paymentStoppedDateIsBlank().clickContinue();
