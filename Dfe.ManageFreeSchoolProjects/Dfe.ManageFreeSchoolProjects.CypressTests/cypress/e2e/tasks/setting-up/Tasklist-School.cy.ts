...
describe('Testing project overview', () => {
    ...
    it('Should successfully set Tasklist-school information', () => {
        ...
        taskListPage.isTaskStatusIsNotStarted('School').selectSchoolFromTaskList();
        ...
        schoolDetailsPage.clickContinue();
        ...
        schoolDetailsPage.withSchoolNameExceedingMaxLength().withFormsOfEntryExceedingLimit().clickContinue();
        ...
        summaryPage
            ...
            .inOrder()
            .summaryShows('Current free school name')
            .HasValue(updatedSchoolName)
            .summaryShows('School type')
            .HasValue('Mainstream')
            .summaryShows('School phase')
            .HasValue('All-through')
            .summaryShows('Age range')
            .HasValue('11-16')
            .summaryShows('Forms of entry')
            .HasValue('3')
            ...
            .summaryShows('Residential or boarding')
            .HasValue('Yes')
            .summaryShows('Alternative provision (specialist resource provision)')
            .HasValue('No')
            .summaryShows('Special educational needs (specialist resource provision)')
            .HasValue('No')
            .summaryShows('Faith status')
        ...
        schoolDetailsPage.withFaithType('faith-type-Other').clickContinue();

        validationComponent.hasValidationError('Enter the other faith type');

        schoolDetailsPage.withFaithTypeOtherDescriptionExceedingMaxLength().clickContinue();

        validationComponent.hasValidationError('Other faith type must be 100 characters or less');

        schoolDetailsPage.withFaithTypeOtherDescription('(This is $invalid)').clickContinue();

        validationComponent.hasValidationError('Other faith type must only contain letters and spaces');

        cy.executeAccessibilityTests();

        schoolDetailsPage.withFaithTypeOtherDescription('This is my faith').clickContinue();
        ...
        schoolDetailsPage.withFaithStatus('None').faithTypeSectionIsNotVisible().clickContinue();
    ...
    it('Should validate the faith type field if a faith status is selected', () => {
        ...
        validationComponent.hasValidationError('Enter the faith type');

        Logger.log('Change faith status to none and back to designation to check validation against faith type none');

        schoolDetailsPage.withFaithStatus('None').withFaithStatus('Designation').clickContinue();

        validationComponent.hasValidationError('Select a different faith type, if faith status is designation.');
    });
