...
describe('Grant letters Task', () => {
    ...
    it('Should successfully add grant letter and variations', () => {
        ...
        pdgDashboard.selectChangeGrantLetter();

        cy.executeAccessibilityTests();

        cy.log('All fields are optional');
        editGrantLetter.grantTitleIs('Edit Grant letters').schoolNameIs(project.schoolName).addGrantLetter();
        cy.executeAccessibilityTests();
        ...
        addGrantLetter
            ...
            .clickContinue()
            .errorForInitialGrantLetterDate()
            .showsError('Day must be a number, like 12')
            .errorForFullGrantLetterDate()
            .showsError('Month must be between 1 and 12');
        cy.executeAccessibilityTests();
        ...
        addGrantLetter
            ...
            .clickContinue();
        editGrantLetter.grantLetterAdded();

        summaryPage
            .inOrder()
            .summaryShows('Date when DfE signed the initial grant letter')
            .HasValue('12 September 2025')
            .summaryShows('Saved the signed initial grant letter in Workplaces folder')
            .HasValue('Yes')
            .summaryShows('Date when DfE signed the final grant letter')
            .HasValue('15 February 2028')
            .summaryShows('Saved the signed full grant letter in Workplaces folder')
            .HasValue('Yes');
        cy.executeAccessibilityTests();

        editGrantLetter.showVariationLabel().addVariation('1');
        ...
        addVariationLetter
            ...
            .clickContinue()
            .errorForDueDate()
            .showsError('Enter a date for the due date of variation letter');
        cy.executeAccessibilityTests();
        ...
        editGrantLetter.variationLetterAdded('1').changeVariationLetter('1');
        cy.executeAccessibilityTests();

        editVariationLetter.variationLetterDueDate('01', '01', '2029').clickContinue();
        cy.executeAccessibilityTests();

        editGrantLetter.showVariationLabel().addVariation('2');

        addVariationLetter.clickDiscard();

        summaryPage
            .inOrder()
            .skip(4)
            .summaryShows('Due date of variation letter')
            .HasValue('1 January 2029')
            .summaryShows('Saved the variation letter in Workplaces folder')
            .HasValue('Yes')
            .clickBack();
