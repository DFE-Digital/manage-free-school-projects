...
describe('Testing the tasks completed count', () => {
    ...
    it('Should be able to set external expert visit', () => {
        ...
        taskListPage.isTaskStatusIsNotStarted('CommissionedExternalExpert').selectExternalExpertVisitFromTaskList();

        Logger.log('Mark External expert visit as completed');

        summaryPage.titleIs('External expert visit').isNotMarkedAsComplete().MarkAsComplete().clickConfirmAndContinue();

        Logger.log('Go back to task list');

        taskListPage
            .taskCompletedCountMessage(`You have completed 1 of ${taskCount} sections.`)
        ...
        taskListPage
            .taskCompletedCountMessage(`You have completed 2 of ${taskCount} sections.`)
            .isTaskStatusIsCompleted('CommissionedExternalExpert')
            .isTaskStatusIsCompleted('ApplicationsEvidence')
            .selectSchoolFromTaskList();

        summaryPage.schoolNameIs(project.schoolName).titleIs('School').clickChange();
        ...
        summaryPage.schoolNameIs(project.schoolName).titleIs('School').clickConfirmAndContinue();

        Logger.log('completed count and sections count is adjusted correctly');

        taskListPage
            .taskCompletedCountMessage(`You have completed 2 of ${taskCount - 1} sections.`)
            .isTaskStatusIsCompleted('CommissionedExternalExpert');
