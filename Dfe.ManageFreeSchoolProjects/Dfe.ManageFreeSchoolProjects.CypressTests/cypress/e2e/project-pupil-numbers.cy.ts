...
describe('Testing the setting of pupil numbers', () => {
    ...
    it('Should be able to edit capacity when full', () => {
        ...
        viewPupilNumbersPage.hasSchoolName(project.schoolName).hasNoSaveBanner();
        ...
        editCapacityWhenFullPage.hasSchoolName(project.schoolName).withNurseryCapacity('asd').saveAndContinue();

        validationComponent.hasValidationError(isNumberValidationMessage.replace('{0}', 'Nursery'));

        editCapacityWhenFullPage.withNurseryCapacity('10000').saveAndContinue();

        validationComponent.hasValidationError(numberOutsideRangeValidationMessage.replace('{0}', 'Nursery'));
    ...
    it('Should be able to edit the pre-16 published admission numbers', () => {
        ...
        Logger.log('Validate fields');
        editPre16PublishedAdmissionNumberPage.withReception('asd').saveAndContinue();

        validationComponent.hasValidationError(isNumberValidationMessage.replace('{0}', 'Reception'));
    ...
    it('Should be able to edit the post-16 published admission numbers', () => {
        ...
        Logger.log('Check all values are 0 initially');
        viewPost16PublishedAdmissionNumberPage.hasYear12('0').hasOtherPost16('0').hasTotal('0');

        viewPupilNumbersPage.editPost16PublishedAdmissionNumber();

        Logger.log('Validate fields');
        editPost16PublishedAdmissionNumberPage.withYear12('asd').saveAndContinue();

        validationComponent.hasValidationError(isNumberValidationMessage.replace('{0}', 'Year 12'));

        editPost16PublishedAdmissionNumberPage.withYear12('-1').withOtherPost16('-1').saveAndContinue();
        ...
        Logger.log('Set valid values');
        editPost16PublishedAdmissionNumberPage.withYear12('12').withOtherPost16('23').saveAndContinue();

        viewPupilNumbersPage.hasSaveBanner();

        viewPost16PublishedAdmissionNumberPage.hasYear12('12').hasOtherPost16('23').hasTotal('35');
    });
    ...
    it('Should be able to edit recruitment and viability', () => {
        ...
        editRecruitmentAndViabilityPage.withReceptionToYear6('asd', 'asd', 'asd').saveAndContinue();
        ...
        validationComponent
            .hasValidationError(
                numberOutsideRangeValidationMessage.replace('{0}', 'Reception to year 6 minimum viable number')
            )
            .hasValidationError(
                numberOutsideRangeValidationMessage.replace('{0}', 'Reception to year 6 applications received')
            )
            .hasValidationError(
                numberOutsideRangeValidationMessage.replace('{0}', 'Reception to year 6 accepted offers')
            )
            .hasValidationError(
                numberOutsideRangeValidationMessage.replace('{0}', 'Year 7 to year 11 minimum viable number')
            )
            .hasValidationError(
                numberOutsideRangeValidationMessage.replace('{0}', 'Year 7 to year 11 applications received')
            )
            .hasValidationError(numberOutsideRangeValidationMessage.replace('{0}', 'Year 7 to year 11 accepted offers'))
            .hasValidationError(
                numberOutsideRangeValidationMessage.replace('{0}', 'Year 12 to year 14 minimum viable number')
            )
            .hasValidationError(
                numberOutsideRangeValidationMessage.replace('{0}', 'Year 12 to year 14 applications received')
            )
            .hasValidationError(
                numberOutsideRangeValidationMessage.replace('{0}', 'Year 12 to year 14 accepted offers')
            );
    ...
    it('Should be able to edit pre-16 capacity buildup', () => {
        ...
        viewPupilNumbersPage.editPre16CapacityBuildup();
        ...
        editCapacityBuildupPage.hasSchoolName(project.schoolName).withNursery('1', '2', '3', '4', '5', '6', '7', '8');

        Logger.log('Configuring reception');

        editCapacityBuildupPage.withReception('-1', '-1', '-1', '-1', '-1', '-1', '-1', '-1').saveAndContinue();

        validateCapacityBuildupRow('Reception');

        editCapacityBuildupPage.withReception('9', '10', '11', '12', '13', '14', '15', '16');

        Logger.log('Configuring year 1');

        editCapacityBuildupPage.withYear1('-1', '-1', '-1', '-1', '-1', '-1', '-1', '-1').saveAndContinue();

        validateCapacityBuildupRow('Year 1');

        editCapacityBuildupPage.withYear1('17', '18', '19', '20', '21', '22', '23', '24');

        Logger.log('Configuring year 2');

        editCapacityBuildupPage.withYear2('-1', '-1', '-1', '-1', '-1', '-1', '-1', '-1').saveAndContinue();

        validateCapacityBuildupRow('Year 2');

        editCapacityBuildupPage.withYear2('25', '26', '27', '28', '29', '30', '31', '32');

        Logger.log('Configuring year 3');

        editCapacityBuildupPage.withYear3('-1', '-1', '-1', '-1', '-1', '-1', '-1', '-1').saveAndContinue();

        validateCapacityBuildupRow('Year 3');

        editCapacityBuildupPage.withYear3('33', '34', '35', '36', '37', '38', '39', '40');

        Logger.log('Configuring year 4');

        editCapacityBuildupPage.withYear4('-1', '-1', '-1', '-1', '-1', '-1', '-1', '-1').saveAndContinue();

        validateCapacityBuildupRow('Year 4');

        editCapacityBuildupPage.withYear4('41', '42', '43', '44', '45', '46', '47', '48');

        Logger.log('Configuring year 5');

        editCapacityBuildupPage.withYear5('-1', '-1', '-1', '-1', '-1', '-1', '-1', '-1').saveAndContinue();

        validateCapacityBuildupRow('Year 5');

        editCapacityBuildupPage.withYear5('49', '50', '51', '52', '53', '54', '55', '56');

        Logger.log('Configuring year 6');

        editCapacityBuildupPage.withYear6('-1', '-1', '-1', '-1', '-1', '-1', '-1', '-1').saveAndContinue();

        validateCapacityBuildupRow('Year 6');

        editCapacityBuildupPage.withYear6('57', '58', '59', '60', '61', '62', '63', '64');

        Logger.log('Configuring year 7');

        editCapacityBuildupPage.withYear7('-1', '-1', '-1', '-1', '-1', '-1', '-1', '-1').saveAndContinue();

        validateCapacityBuildupRow('Year 7');

        editCapacityBuildupPage.withYear7('65', '66', '67', '68', '69', '70', '71', '72');

        Logger.log('Configuring year 8');

        editCapacityBuildupPage.withYear8('-1', '-1', '-1', '-1', '-1', '-1', '-1', '-1').saveAndContinue();

        validateCapacityBuildupRow('Year 8');

        editCapacityBuildupPage.withYear8('73', '74', '75', '76', '77', '78', '79', '80');

        Logger.log('Configuring year 9');

        editCapacityBuildupPage.withYear9('-1', '-1', '-1', '-1', '-1', '-1', '-1', '-1').saveAndContinue();

        validateCapacityBuildupRow('Year 9');

        editCapacityBuildupPage.withYear9('81', '82', '83', '84', '85', '86', '87', '88');

        Logger.log('Configuring year 10');

        editCapacityBuildupPage.withYear10('-1', '-1', '-1', '-1', '-1', '-1', '-1', '-1').saveAndContinue();

        validateCapacityBuildupRow('Year 10');

        editCapacityBuildupPage.withYear10('89', '90', '91', '92', '93', '94', '95', '96');

        Logger.log('Configuring year 11');

        editCapacityBuildupPage.withYear11('-1', '-1', '-1', '-1', '-1', '-1', '-1', '-1').saveAndContinue();

        validateCapacityBuildupRow('Year 11');

        editCapacityBuildupPage.withYear11('97', '98', '99', '100', '101', '102', '103', '104');
    ...
    it('Should be able to edit post-16 capacity buildup', () => {
        ...
        viewPupilNumbersPage.editPost16CapacityBuildup();
        ...
        editCapacityBuildupPage.hasSchoolName(project.schoolName).withYear12('2', '3', '4', '5', '6', '7', '8', '9');

        Logger.log('Configuring year 13');

        editCapacityBuildupPage.withYear13('-1', '-1', '-1', '-1', '-1', '-1', '-1', '-1').saveAndContinue();

        validateCapacityBuildupRow('Year 13');

        editCapacityBuildupPage.withYear13('10', '11', '12', '13', '14', '15', '16', '17');

        Logger.log('Configuring year 14');

        editCapacityBuildupPage.withYear14('-1', '-1', '-1', '-1', '-1', '-1', '-1', '-1').saveAndContinue();

        validateCapacityBuildupRow('Year 14');

        editCapacityBuildupPage.withYear14('18', '19', '20', '21', '22', '23', '24', '25');
