...
describe('Testing that we can add a project risk', () => {
    ...
    describe('Adding a project risk', () => {
        it('Should be able to add project risk', () => {
            ...
            editProjectRiskPage.withGovernanceAndSuitabilityRiskSummaryExceeding().continue();

            validationComponent.hasValidationError('Summary must be 1000 characters or less');
            ...
            editProjectRiskPage.withEducationSummaryExceeding().continue();
            ...
            editProjectRiskPage.withFinanceSummaryExceeding().continue();
            ...
            editProjectRiskPage.withRiskAppraisalFormSharePointLink('aaaa').continue();

            validationComponent.hasValidationError('SharePoint link must be a valid url');

            editProjectRiskPage.withRiskAppraisalFormSharePointLinkExceeding().continue();
            ...
            editProjectRiskPage.withOverallRiskSummaryExceeding().continue();

            validationComponent.hasValidationError('Summary must be 5000 characters or less');
        ...
        it('Should be able to add multiple project risks with the latest being displayed by default', () => {
            Logger.log('Changing project risk');
            projectOverviewPage.selectRiskRatingAndSummaryTab().changeProjectRisk();
            ...
            projectRiskHistoryTable
                .getRowByIndex(1)
                .then((firstRow) => {
                    firstRow.hasDate(toDisplayDate(now)).hasRiskRating(['Amber', 'Green']).hasNoViewLink();

                    return projectRiskHistoryTable.getRowByIndex(2);
                })
                .then((secondRow) => {
                    secondRow.hasDate(toDisplayDate(now)).hasRiskRating(['Green']);
        ...
        it('Should be able to add a project risk with the minimum values set', () => {
            ...
            projectRiskHistoryTable.getRowByIndex(1).then((firstRow) => {
                firstRow.hasDate(toDisplayDate(now)).hasRiskRating(['Green']);
            });
        });
        ...
        it('Should add a project risk entry even if none of the data changes', () => {
            ...
            projectRiskSummaryPage.hasOverallRiskRating(['Green']);
