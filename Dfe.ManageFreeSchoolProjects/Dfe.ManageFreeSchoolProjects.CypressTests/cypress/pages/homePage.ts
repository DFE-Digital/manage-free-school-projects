class HomePage {
    ...
    public hasProjectFilter(project: string): this {
        cy.getByTestId('search-by-project').should('have.value', project);
...
*/
    ...
    public tryViewProjectWithFilters(): this {
        ...
        cy.get('tbody')
            ...
            .filter((_, row) => {
                ...
                const hasProjectTitleOrId =
                    $row.find(dataTestIds.projectTitle).text().trim() !== '' ||
            ...
            .then((firstRow) => {
                ...
                this.FilterData.projectTitle =
                    Cypress.$(firstRow).find(dataTestIds.projectTitle).text().trim() ||
    ...
    public regionsFilterInputCleared(): this {
        cy.get('#search-by-region-container input').each(($input) => {
    ...
    public selectAllLocalAuthorities(): this {
        cy.get('#la-checkbox-container input').each(($input) => {
            cy.wrap($input).check();
        });
    ...
    public withAllProjectAssignedTo(): this {
        cy.get('#search-by-pmb-container input').each(($input) => {
            cy.wrap($input).check();
        });
